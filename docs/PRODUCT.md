...
- **The board** (`node checks/progress.mjs --serve`), a set of local pages that render what the
  project is for, what is being built, and every document it holds: the way in, the lanes, the
  round, the features, one feature, and a folder index for a place holding more documents than a
  sidebar will list. Every one of them carries the same sidebar, so a followed link is never a
  dead end. Composed by `checks/board-page.mjs` and `checks/board-views.mjs`, navigated by
  `checks/board-nav.mjs`, generated on demand, never stored, and read by the owner of a project
  rather than by a visitor. The whole of it printed as one self-contained file
  (`node checks/progress.mjs --page`) is the second output of that one derivation: the picture at
  the moment it was made, for a reader who has no repository beside it, so it opens nothing,
  fetches nothing, and carries no navigation, having nowhere inside one file to navigate to.
...
- The board is rendered by a check script, so its styling lives in that script rather than being
  read from the token section of `docs/DESIGN.md`, which is still unwritten. Since 2026-08-25 the
  two surfaces no longer even agree: the explainer keeps its own palette, and the board was
  rebuilt on a reference the owner supplied, charcoal with one peach accent. That is the known
  debt on this surface, and it is now a drift rather than a risk of one: `docs/state/DEBT.md`,
  DEBT-001.
- The board holds still while it is used: the page itself does not scroll, the containers in it
  do. Below the stacking breakpoint, and on paper, it scrolls the ordinary way, because a stacked
  sidebar above a locked page would put everything under it out of reach.
