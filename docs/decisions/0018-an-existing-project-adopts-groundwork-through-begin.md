...
4. **A script that installs Groundwork into an existing repo:** not on the table. The out-of-scope
   ...
   the brief now allows a few zero-dependency scripts (decision 0022), so adoption will run
   through `init --adopt` plus `begin`: the script lays the framework files down, and `begin`
   keeps the interview and the brownfield branch below. Until `init` is built, the route below is
   the whole route, first measurement included.
